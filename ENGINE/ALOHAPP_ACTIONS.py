...
rec_path = path_beginning+"DATA/ALOHA/PHRASES/SPEAKING/"

def pic_button_clicked(e):
    cam = cv.VideoCapture(0)
    result, image = cam.read()

    if result:
        cv.imwrite("test_kamery.jpg", image)
        print("Image saved!")
...
def rec_button_clicked(e):
    ...
    mic_recorder = Mic_Recorder(length=4,path=rec_path, file='flet.wav')
    mic_recorder.record()
    ...
    if e.page.main_language=='de': bot_text = e.page.chat_bot.talk('Auf einen Satz antworten:'+whisper_model.last_transcribe+'mit diesen Worten'+words+' in maximal 85 Zeichen')

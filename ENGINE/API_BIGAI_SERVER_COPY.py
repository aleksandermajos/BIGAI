...
if STT_WHISPERX:
    ...
    if os_name == 'Linux':
        import whisperx
        model = whisperx.load_model("large-v3", device="cuda",device_index=1)
...
if SPACY_STANZA:
    ...
    if os_name == 'Linux':
        lemma_pl = spacy_stanza.load_pipeline('pl',device='cuda')
        lemma_en = spacy_stanza.load_pipeline('en',device='cuda')
        lemma_ja = spacy_stanza.load_pipeline('ja', device='cuda')
        lemma_zh = spacy_stanza.load_pipeline('zh', device='cuda')
